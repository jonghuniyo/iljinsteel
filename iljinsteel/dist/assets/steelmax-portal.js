(function () {
  ...
  function renderSelected() {
    ...
    if (!main) return;
    if (!state.selected) {
      main.innerHTML = `<div class="smx-empty"><div><strong style="font-size:18px;color:#0f172a">Steelmax 자료실</strong><br/>왼쪽에서 철강·강관·규격 키워드를 검색하면 원문을 포털 안에서 열람하고 저장할 수 있습니다.<br/><br/><span style="font-size:12px">권한 받은 개인용 이전 작업 기준 · 원문 출처 링크 유지</span></div></div>`;
      return;
    }
  ...
  function mount() {
    ...
    btn.className = "smx-open-btn";
    btn.innerHTML = "Steelmax 자료실<small>철강·강관 검색</small>";
    ...
    overlay.innerHTML = `
      <section class="smx-panel" role="dialog" aria-modal="true" aria-label="Steelmax 자료실">
        <aside class="smx-left">
          <div class="smx-head"><div><h1 class="smx-title">Steelmax 자료실</h1><div class="smx-sub">허락받은 자료를 개인 포털에서 검색·열람·저장합니다. 원문 출처 링크는 유지됩니다.</div></div><button class="smx-close" id="smxClose" aria-label="닫기">×</button></div>
